...
public class MaxSumTriangleGridMemoization {
    ...
    public static int findPaths(int row, int col, int[][] arr, int[][] dp){
        ...
        int down = arr[row][col] + findPaths(row+1, col, arr, dp);
        int diag = arr[row][col] + findPaths(row+1, col+1, arr, dp);

        return dp[row][col] = Math.max(down,diag);
    }
