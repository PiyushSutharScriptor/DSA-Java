...
public class MaxSumTriangleGridRecursion {
    ...
    public static int findPaths(int row, int col, int[][] arr){
        ...
        int down = arr[row][col] + findPaths(row+1, col, arr);
        int diag = arr[row][col] + findPaths(row+1, col+1, arr);

        return Math.max(down,diag);
    }
